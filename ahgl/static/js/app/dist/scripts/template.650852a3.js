angular.module('ahglApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/header.html',
    "<div ng-controller=HeaderCtrl><a href=\"/\"><div class=logo></div></a><div class=bar><div ng-repeat=\"game in games\"><a ng-href=\"/#/tournament/{{ game.tournament_slug }}\"><img class=\"game1 game\" ng-src=\"{{ getImageUrl(game) }}\"></a> <img class=game-active-icon ng-if=isSelected(game) ng-src=\"{{ getSelectedIconUrl(game) }}\"></div></div><div class=navigation ng-show=isGameSelected><ul><li ng-repeat=\"tab in tabs\"><a href=\"/{{ selectedGame }}/{{ tab.slug }}\" ng-class=\"{ selected: selectedTab == tab.slug }\">{{ tab.title }}</a></li></ul></div></div>"
  );


  $templateCache.put('views/latestNews.html',
    "<div ng-controller=LatestNewsCtrl ng-show=newsPresent><div class=header><img class=section-header-icon ng-src=\"{{ sectionHeaderIconUrl }}\"> <span class=heavy>Latest</span><span>Ahgl News</span></div><div class=latest-news><div ng-repeat=\"article in news\" class=news><div class=news-heading><div class=date>{{ formatDate(article) }}</div><img ng-show=\"{{ isVisible(article) }}\" ng-src=\"{{ article.icon_image_url }}\"><div>{{ article.title }}</div><div class=underline></div></div><p ng-bind-html=article.summary></p><a href=\"{{ article.page_url }}\">Read More</a></div></div></div>"
  );
